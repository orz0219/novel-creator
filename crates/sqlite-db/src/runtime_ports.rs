...
pub struct DbNarrativePort { pool: SqlitePool }
...
pub struct DbEntityPort { pool: SqlitePool }
...
pub struct DbStatePort { pool: SqlitePool }
...
pub struct DbKnowledgePort { pool: SqlitePool }
...
pub struct DbRelationPort { pool: SqlitePool }
...
impl RelationPort for DbRelationPort {
    async fn find_relations_by_entities(&self, project_id: Uuid, entity_ids: &[Uuid]) -> Result<Vec<Relation>> {
        let rows = sqlx::query_as::<_, RelationRow>("SELECT id, project_id, source_entity_id, target_entity_id, relation_type, description, attributes, valid_from, valid_until, created_at, updated_at FROM relation WHERE project_id = $1 AND (upper(replace(hex(source_entity_id), '-', '')) IN (SELECT upper(replace(value, '-', '')) FROM json_each($2)) OR upper(replace(hex(target_entity_id), '-', '')) IN (SELECT upper(replace(value, '-', '')) FROM json_each($2))) ORDER BY created_at DESC")
            .bind(project_id).bind(serde_json::to_string(&entity_ids).context("序列化 id 数组失败")?).fetch_all(&self.pool).await
            .context("Failed to query relations - critical database error")?;
        Ok(rows.into_iter().map(to_relation).collect())
    }
}

pub struct DbEventPort { pool: SqlitePool }

#[async_trait::async_trait]
impl EventPort for DbEventPort {
    async fn find_events_by_entities(&self, project_id: Uuid, entity_ids: &[Uuid]) -> Result<Vec<Event>> {
        let rows = sqlx::query_as::<_, EventRow>("SELECT DISTINCT e.id, e.project_id, e.name, e.description, e.event_type, e.event_time, e.duration, e.created_at, e.updated_at FROM event e INNER JOIN event_entity ee ON e.id = ee.event_id WHERE e.project_id = $1 AND upper(replace(hex(ee.entity_id), '-', '')) IN (SELECT upper(replace(value, '-', '')) FROM json_each($2)) ORDER BY e.created_at DESC LIMIT 20")
            .bind(project_id).bind(serde_json::to_string(&entity_ids).context("序列化 id 数组失败")?).fetch_all(&self.pool).await
...
pub struct DbCanonRulePort { pool: SqlitePool }
...
pub struct DbContextSnapshotPort { pool: SqlitePool }
...
pub struct DbValidationPort { pool: SqlitePool }
...
pub struct DbApprovalPort { pool: SqlitePool }
...
pub struct DbProposedChangeQueryPort { pool: SqlitePool }
...
// ---------------------------------------------------------------------------
// Constructors: each port is built from a SqlitePool. These are the only place the
