...
impl NarrativeRepo {
    ...
    pub async fn update_node_tx(
        ...
        let result = sqlx::query(
            "UPDATE narrative_node SET title=$1, description=$2, content=$3, attributes=$4, sort_order=$5, status=$6, version=version+1, updated_at=CURRENT_TIMESTAMP \
    ...
    pub async fn soft_delete_node_tx(
        ...
        let result = sqlx::query(
            "WITH RECURSIVE sub(id) AS ( \
             ...
             ) UPDATE narrative_node SET status = 'Deleted', updated_at = CURRENT_TIMESTAMP WHERE id IN (SELECT id FROM sub)",
        )
        .bind(id)
        .execute(executor)
        .await
