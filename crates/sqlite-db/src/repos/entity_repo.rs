...
impl EntityTypeRepo {
    ...
    pub async fn ensure_tx<'c>(
        executor: impl sqlx::Executor<'c, Database = sqlx::Sqlite>,
        name: &str,
        description: Option<&str>,
    ) -> Result<EntityType> {
        let row = sqlx::query_as::<_, EntityTypeRow>(
            "INSERT INTO entity_type (id, name, description, created_at, updated_at)
             VALUES (randomblob(16), $1, $2, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
...
impl EntityRepo {
    ...
    pub async fn list_by_ids(&self, project_id: Uuid, ids: &[Uuid]) -> Result<Vec<Entity>> {
        ...
        let rows = sqlx::query_as::<_, EntityRow>(
            "SELECT id, project_id, world_id, entity_type_id, name, summary, description, \
             attributes, version, created_by, updated_by, source_generation_id, created_at, updated_at \
             FROM entity WHERE project_id = $1 AND upper(replace(hex(id), '-', '')) IN (SELECT upper(replace(value, '-', '')) FROM json_each($2)) ORDER BY name",
        )
        .bind(project_id)
    ...
    pub async fn update(&self, entity: &Entity) -> Result<()> {
        let result = sqlx::query(
            "UPDATE entity SET name = $1, summary = $2, description = $3, attributes = $4, \
             version = version + 1, updated_by = 'agent', updated_at = CURRENT_TIMESTAMP \
    ...
    pub async fn create_tx<'c>(
        ...
        sqlx::query(
            "INSERT INTO entity (id, project_id, world_id, entity_type_id, name, summary, description, attributes, version, created_by, created_at, updated_at) \
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 1, 'system', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)",
        )
        .bind(id)
        .bind(project_id)
        .bind(world_id)
    ...
    pub async fn update_tx<'c>(
        ...
        let result = sqlx::query(
            "UPDATE entity SET name = $1, summary = $2, description = $3, attributes = $4, \
             version = version + 1, updated_by = $5, updated_at = CURRENT_TIMESTAMP \
...
impl RelationRepo {
    ...
    pub async fn end_relation_tx<'c>(
        ...
        let result = match valid_until {
            Some(vu) => sqlx::query(
                "UPDATE relation SET valid_until = $3, updated_at = CURRENT_TIMESTAMP \
            ...
            None => sqlx::query(
                "UPDATE relation SET valid_until = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP \
                 WHERE id = $1 AND project_id = $2 AND valid_until IS NULL",
            )
            .bind(id)
            .bind(project_id)
...
impl FactRepo {
    ...
    pub async fn create_tx(
        ...
        for entity_id in related_entity_ids {
            sqlx::query("INSERT INTO fact_entity (id, fact_id, entity_id) VALUES (randomblob(16), $1, $2)")
                .bind(id)
                .bind(entity_id)
                .execute(&mut *executor)
                .await
    ...
    pub async fn get_by_id_with_project_tx<'c>(
        executor: impl sqlx::Executor<'c, Database = sqlx::Sqlite> + Copy,
        project_id: Uuid,
        id: Uuid,
    ) -> Result<Option<Fact>> {
        let row = sqlx::query_as::<_, FactRow>(
    ...
    pub async fn set_status_tx(
        ...
        let result = sqlx::query(
            "UPDATE fact SET status = $3, superseded_by = $4, updated_at = CURRENT_TIMESTAMP \
