...
impl ForeshadowingRepo {
    ...
    pub async fn update_status(&self, id: Uuid, status: ForeshadowingStatus) -> Result<()> {
        ...
        sqlx::query("UPDATE foreshadowing SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2")
            .bind(status_str)
            .bind(id)
            .execute(&self.pool)
            .await
    ...
    pub async fn update_tx(
        ...
        let result = sqlx::query(
            "UPDATE foreshadowing SET name = COALESCE($1, name), description = COALESCE($2, description), status = COALESCE($3, status), updated_at = CURRENT_TIMESTAMP \
