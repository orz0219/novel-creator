...
//! Transaction-aware methods (suffixed _tx) accept a &mut SqliteConnection
...
impl StateRepo {
    ...
    pub async fn list_current_states_batch(
        ...
        let rows = sqlx::query_as::<_, CurrentStateRow>(
            "SELECT id, project_id, entity_id, state_key, state_value, effective_from, effective_to, version, created_at, updated_at \
             FROM current_state WHERE project_id = $1 AND upper(replace(hex(entity_id), '-', '')) IN (SELECT upper(replace(value, '-', '')) FROM json_each($2)) AND effective_to IS NULL ORDER BY entity_id, state_key",
        )
        .bind(project_id)
        .bind(serde_json::to_string(&entity_ids).context("序列化 id 数组失败")?)
        .fetch_all(&self.pool)
        .await
