...
impl ApprovalRepo {
    ...
    pub async fn approve(&self, id: Uuid, reviewer_id: &str, comment: Option<&str>) -> Result<()> {
        sqlx::query(
            "UPDATE approval_record SET status = 'Approved', reviewer_id = $1, reviewer_comment = $2, reviewed_at = CURRENT_TIMESTAMP WHERE id = $3",
        )
        .bind(reviewer_id)
    ...
    pub async fn reject(&self, id: Uuid, reviewer_id: &str, comment: Option<&str>) -> Result<()> {
        sqlx::query(
            "UPDATE approval_record SET status = 'Rejected', reviewer_id = $1, reviewer_comment = $2, reviewed_at = CURRENT_TIMESTAMP WHERE id = $3",
        )
        .bind(reviewer_id)
        .bind(comment.unwrap_or(""))
        .bind(id)
