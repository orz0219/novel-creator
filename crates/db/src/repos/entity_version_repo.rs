...
impl EntityVersionRepo {
    ...
    pub async fn list(pool: &PgPool, entity_id: Uuid) -> Result<Vec<Revision>> {
        let rows = sqlx::query(
    ...
    /// 版本列表（含每一版相对上一版的差异）
    pub async fn list_entries(pool: &PgPool, entity_id: Uuid) -> Result<Vec<Value>> {
    ...
    /// 单版详情
    pub async fn get_entry(pool: &PgPool, entity_id: Uuid, version: i32) -> Result<Option<Value>> {
        let revs = Self::list(pool, entity_id).await?;
...
impl LocationLinksRepo {
    ...
    pub async fn entities(pool: &PgPool, location_id: Uuid) -> Result<Vec<Value>> {
        let rows = sqlx::query(
    ...
    /// 与该地点相关联的事件
    pub async fn events(pool: &PgPool, location_id: Uuid) -> Result<Vec<Value>> {
        let rows = sqlx::query(
