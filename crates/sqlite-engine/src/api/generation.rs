...
use std::sync::Arc;
use sqlite_db::application_ports::{DbEntityRepositoryPort, DbGenerationRepositoryPort, DbNarrativeRepositoryPort};
...
fn generation_executor(state: &AppState) -> GenerationExecutor {
    ...
    let prop_repo = Arc::new(sqlite_db::application_ports::DbProposalRepositoryPort::new(pool.clone()));
    let snapshots = Arc::new(sqlite_db::application_ports::DbContextSnapshotRepositoryPort::new(
        pool.clone(),
    ));
...
async fn build_scene_context(pool: &sqlx::SqlitePool, task_id: Uuid) -> Option<String> {
