...
pub fn router(state: AppState) -> Router {
    ...
    Router::new()
        ...
        // 电脑端导出的项目数据导入（手机单机版专有端点）
        .route("/api/v1/import", post(import_project))
        // 引擎就绪状态（前端据此决定是否显示首次配置引导）
        .route("/api/v1/engine/ready", get(engine_ready))
        // 接收手机备份（手机 → 电脑，局域网内使用）
...
) -> Result<axum::Json<sqlite_db::export::ProjectExport>, crate::api::error::AppError> {
    let export = sqlite_db::export::export_project(&state.pool, &id).await?;
    Ok(axum::Json(export))
}

/// `GET /api/v1/engine/ready` —— 引擎是否已可用（含 AI 网关是否配置）。
///
/// 同时回传**当前生效**的配置值（`config`）：手机端首次启动时密钥来自构建注入
/// （见 `mobile/src-tauri/build.rs`），并不在数据库里；设置页需要它把
/// 「当前生效的密钥」显示出来，否则用户会以为密钥没带过来。
/// 本接口只监听 127.0.0.1，仅 App 自身的 WebView 能访问。
async fn engine_ready(
    State(state): State<AppState>,
) -> Result<axum::Json<serde_json::Value>, error::AppError> {
    let cfg = state.ai_settings.load().await?;
    let configured = !cfg.api_key.as_deref().unwrap_or("").trim().is_empty();
    Ok(axum::Json(serde_json::json!({
        "ok": true,
        ...
        "config": {
            ...
            "aiApiKey": cfg.api_key.clone().unwrap_or_default(),
...
/// `POST /api/v1/import` —— 导入电脑端导出的项目 JSON（请求体即导出文件原文）。
async fn import_project(
    State(state): State<AppState>,
    body: String,
) -> Result<axum::Json<sqlite_db::import::ImportSummary>, error::AppError> {
    let summary = sqlite_db::import::import_from_json(&state.pool, &body).await?;
