...
use sqlite_db::application_ports::{DbStorylineRepositoryPort, DbForeshadowRepositoryPort};
...
fn narrative_service(state: &AppState) -> NarrativeService {
    ...
    NarrativeService::new(
        Arc::new(sqlite_db::application_ports::DbNarrativeRepositoryPort::new(pool)),
        committer,
        resolver,
    )
}
